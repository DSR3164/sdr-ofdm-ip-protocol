...
void ip_dev(App &app, Buffers &data) // IP layer
{
    ...
    if (ImGui::Begin("IP Layer"))
    {
        ...
        ImGui::Text("[IP] Ver=%d HLen=%d TOS=%02X Len=%d Proto=%d Src=%d.%d.%d.%d Dest=%d.%d.%d.%d",
            (raw_ptr_bytes[9] >> 4) & 0xF,
            (raw_ptr_bytes[9] & 0xF) * 4,
            raw_ptr_bytes[10],
            (raw_ptr_bytes[11] << 8) | raw_ptr_bytes[12],
            raw_ptr_bytes[18],
            raw_ptr_bytes[21], raw_ptr_bytes[22],
            raw_ptr_bytes[23], raw_ptr_bytes[24],
            raw_ptr_bytes[25], raw_ptr_bytes[26],
            raw_ptr_bytes[27], raw_ptr_bytes[28]);
